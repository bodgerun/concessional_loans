...
| PostgreSQL | `localhost:5432` (user/password/db: `postgres` / `postgres` / `concessional_loans`) |
...
- внутри Compose хост БД в URL — `db` (имя сервиса), не `localhost`;
- для локального запуска вне Docker (миграции, отладка) обычно нужен `...@localhost:5432/...`;
